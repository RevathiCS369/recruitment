...
// Division_Choice_PS is the model entity for the Division_Choice_PS schema.
...
func (*Division_Choice_PS) scanValues(columns []string) ([]any, error) {
	values := make([]any, len(columns))
	for i := range columns {
		switch columns[i] {
		case division_choice_ps.FieldID, division_choice_ps.FieldEmployeeID:
			values[i] = new(sql.NullInt64)
		case division_choice_ps.FieldApplication, division_choice_ps.FieldCadrePrefNo, division_choice_ps.FieldCadrePrefValue, division_choice_ps.FieldUpdatedBy:
...
// to the Division_Choice_PS fields.
func (dcp *Division_Choice_PS) assignValues(columns []string, values []any) error {
	if m, n := len(values), len(columns); m < n {
		return fmt.Errorf("mismatch number of scan values: %d != %d", m, n)
	}
	for i := range columns {
		switch columns[i] {
		case division_choice_ps.FieldID:
...
// Value returns the ent.Value that was dynamically selected and assigned to the Division_Choice_PS.
// This includes values selected through modifiers, order, etc.
func (dcp *Division_Choice_PS) Value(name string) (ent.Value, error) {
	return dcp.selectValues.Get(name)
}

// Update returns a builder for updating this Division_Choice_PS.
// Note that you need to call Division_Choice_PS.Unwrap() before calling this method if this Division_Choice_PS
// was returned from a transaction, and the transaction was committed or rolled back.
func (dcp *Division_Choice_PS) Update() *DivisionChoicePSUpdateOne {
	return NewDivisionChoicePSClient(dcp.config).UpdateOne(dcp)
}

// Unwrap unwraps the Division_Choice_PS entity that was returned from a transaction after it was closed,
// so that all future queries will be executed through the driver which created the transaction.
func (dcp *Division_Choice_PS) Unwrap() *Division_Choice_PS {
	_tx, ok := dcp.config.driver.(*txDriver)
	if !ok {
		panic("ent: Division_Choice_PS is not a transactional entity")
	}
...
func (dcp *Division_Choice_PS) String() string {
	var builder strings.Builder
	builder.WriteString("Division_Choice_PS(")
...
// Division_Choice_PSs is a parsable slice of Division_Choice_PS.
type Division_Choice_PSs []*Division_Choice_PS
