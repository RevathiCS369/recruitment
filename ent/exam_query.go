...
func (eq *ExamQuery) QueryNodalOfficers() *NodalOfficerQuery {
	query := (&NodalOfficerClient{config: eq.config}).Query()
...
func (eq *ExamQuery) WithNodalOfficers(opts ...func(*NodalOfficerQuery)) *ExamQuery {
	query := (&NodalOfficerClient{config: eq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eq.withNodalOfficers = query
	return eq
}
...
func (eq *ExamQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Exam, error) {
	...
	if query := eq.withNodalOfficers; query != nil {
		if err := eq.loadNodalOfficers(ctx, query, nodes,
			func(n *Exam) { n.Edges.NodalOfficers = []*NodalOfficer{} },
			func(n *Exam, e *NodalOfficer) { n.Edges.NodalOfficers = append(n.Edges.NodalOfficers, e) }); err != nil {
			return nil, err
		}
	}
...
func (eq *ExamQuery) loadNodalOfficers(ctx context.Context, query *NodalOfficerQuery, nodes []*Exam, init func(*Exam), assign func(*Exam, *NodalOfficer)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(nodalofficer.FieldExamCode)
	}
	query.Where(predicate.NodalOfficer(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(exam.NodalOfficersColumn), fks...))
	}))
