...
type ExamApplicationsIPQuery struct {
	...
	withExamApplnIPRef    *ExamIPQuery
	withOfficeIPRef       *FacilityQuery
...
func (eaiq *ExamApplicationsIPQuery) QueryExamApplnIPRef() *ExamIPQuery {
	query := (&ExamIPClient{config: eaiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eaiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eaiq *ExamApplicationsIPQuery) QueryOfficeIPRef() *FacilityQuery {
	query := (&FacilityClient{config: eaiq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := eaiq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (eaiq *ExamApplicationsIPQuery) Clone() *ExamApplicationsIPQuery {
	...
	return &ExamApplicationsIPQuery{
		...
		withExamApplnIPRef:    eaiq.withExamApplnIPRef.Clone(),
		withOfficeIPRef:       eaiq.withOfficeIPRef.Clone(),
...
func (eaiq *ExamApplicationsIPQuery) WithExamApplnIPRef(opts ...func(*ExamIPQuery)) *ExamApplicationsIPQuery {
	query := (&ExamIPClient{config: eaiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eaiq.withExamApplnIPRef = query
	return eaiq
}
...
func (eaiq *ExamApplicationsIPQuery) WithOfficeIPRef(opts ...func(*FacilityQuery)) *ExamApplicationsIPQuery {
	query := (&FacilityClient{config: eaiq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	eaiq.withOfficeIPRef = query
	return eaiq
}
...
func (eaiq *ExamApplicationsIPQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Exam_Applications_IP, error) {
	var (
		...
		loadedTypes = [7]bool{
			...
			eaiq.withExamApplnIPRef != nil,
			eaiq.withOfficeIPRef != nil,
	...
	if query := eaiq.withExamApplnIPRef; query != nil {
		if err := eaiq.loadExamApplnIPRef(ctx, query, nodes,
			func(n *Exam_Applications_IP) { n.Edges.ExamApplnIPRef = []*Exam_IP{} },
			func(n *Exam_Applications_IP, e *Exam_IP) { n.Edges.ExamApplnIPRef = append(n.Edges.ExamApplnIPRef, e) }); err != nil {
			return nil, err
		}
	}
	if query := eaiq.withOfficeIPRef; query != nil {
		if err := eaiq.loadOfficeIPRef(ctx, query, nodes,
			func(n *Exam_Applications_IP) { n.Edges.OfficeIPRef = []*Facility{} },
			func(n *Exam_Applications_IP, e *Facility) { n.Edges.OfficeIPRef = append(n.Edges.OfficeIPRef, e) }); err != nil {
			return nil, err
		}
	}
...
func (eaiq *ExamApplicationsIPQuery) loadExamApplnIPRef(ctx context.Context, query *ExamIPQuery, nodes []*Exam_Applications_IP, init func(*Exam_Applications_IP), assign func(*Exam_Applications_IP, *Exam_IP)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Exam_Applications_IP)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Exam_IP(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(exam_applications_ip.ExamApplnIPRefColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.exam_applications_ip_exam_appln_ip_ref
		if fk == nil {
			return fmt.Errorf(`foreign-key "exam_applications_ip_exam_appln_ip_ref" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "exam_applications_ip_exam_appln_ip_ref" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (eaiq *ExamApplicationsIPQuery) loadOfficeIPRef(ctx context.Context, query *FacilityQuery, nodes []*Exam_Applications_IP, init func(*Exam_Applications_IP), assign func(*Exam_Applications_IP, *Facility)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int64]*Exam_Applications_IP)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Facility(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(exam_applications_ip.OfficeIPRefColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.exam_applications_ip_office_ip_ref
		if fk == nil {
			return fmt.Errorf(`foreign-key "exam_applications_ip_office_ip_ref" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "exam_applications_ip_office_ip_ref" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
...
func (eaiq *ExamApplicationsIPQuery) loadCirclePrefRef(ctx context.Context, query *PlaceOfPreferenceIPQuery, nodes []*Exam_Applications_IP, init func(*Exam_Applications_IP), assign func(*Exam_Applications_IP, *PlaceOfPreferenceIP)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(placeofpreferenceip.FieldApplicationID)
	}
...
func (eaiq *ExamApplicationsIPQuery) loadIPApplicationsRef(ctx context.Context, query *RecommendationsIPApplicationsQuery, nodes []*Exam_Applications_IP, init func(*Exam_Applications_IP), assign func(*Exam_Applications_IP, *RecommendationsIPApplications)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(recommendationsipapplications.FieldApplicationID)
	}
